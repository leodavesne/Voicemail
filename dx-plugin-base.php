...
class DX_Plugin_Base {
	...
	public function __construct() {
		...
		add_action( 'init', array( $this, 'dx_sample_shortcode_voicemail' ) );
...
*/
	...
	public function dx_add_JS() {
		...
		wp_register_script( 'audiodisplay', plugins_url( '/js/audiodisplay.js' , __FILE__ ));
		wp_enqueue_script( 'audiodisplay' );
		*/

		// wp_register_script( 'recorderjs', plugins_url( '/js/recorderjs/recorder.js' , __FILE__ ));
		// wp_enqueue_script( 'recorderjs' );

		/*
		$data = array(
			'RECORDER_WORKER_PATH' => plugins_url("/js/recorderjs/recorderWorker.js", __FILE__ ),
			'ajaxurl' => plugins_url('getAudio.php', __FILE__ )
		);

		wp_enqueue_script("recorderjs", plugins_url( '/js/recorderjs/recorder.js' , __FILE__ ));
		wp_localize_script("recorderjs", "variables", $data);
		*/
		...
		wp_enqueue_script('my_js_library', get_template_directory_uri() . '/js/voice.js');
		...
		*/
		...
		$data = array(
			'ajaxurl' => plugins_url('getAudio.php', __FILE__ ),
			'PLUGIN_PATH' => plugins_url("dx-plugin-base.php", __FILE__ )
		);

	    wp_enqueue_script("voice", plugins_url("/js/voice.js", __FILE__ ));
		...
		*/
		...
		# wp_register_script( 'recorder', plugins_url( '/projects/Recorderjs/dist/recorder.js' , __FILE__ ));
		# wp_enqueue_script( 'recorder' );
	}
	...
	public function dx_sample_shortcode_voicemail() {
		add_shortcode( 'voicemail', array( $this, 'dx_sample_shortcode_voicemail_body' ) );
	}
	...
	public function dx_sample_shortcode_body( $attr, $content = null ) {
		...
		return __(readfile(plugins_url( 'record.html' , __FILE__ )), 'dxbase');
	}

	public function dx_sample_shortcode_voicemail_body( $attr, $content = null ) {
		/*
		 * Manage the attributes and the content as per your request and return the result
		 */

		// return __( 'Sample Output', 'dxbase');
		return __(readfile(plugins_url( '/html/voicemail.html' , __FILE__ )), 'dxbase');
	}
...
function insert_my_footer() {
	...
	echo '<br />';
	echo plugins_url( 'record.html' , __FILE__ );

	echo readfile(plugins_url( 'record.html' , __FILE__ ));
	*/
}
